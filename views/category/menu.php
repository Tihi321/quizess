...
<div class="<?php echo esc_attr( $class_category_menu ); ?>">
  <?php
      ...
      $active    = ( is_post_type_archive( Config::QUIZESS_POST_SLUG ) ) ? " {$is_active_class}" : '';
  ?>
  ...
  <?php
  if ( ! empty( $categories ) ) {
    if ( is_post_type_archive( Config::QUIZESS_POST_SLUG ) ) {
      foreach ( $categories as $item ) {
        $category_item = $base_path . 'views/category/parts/home-item.php';
